...
- **Solver/**
  旅行采购员问题的 Visual C++ 项目.
  核心算法.

- **Protocol/**
  - TravelingPurchaser.proto
    旅行采购员问题的输入输出数据定义文件.
  - ShortestPassWithMustPassNodes.proto
    必经点最短路问题的输入数据定义文件. 用于将该问题的算例转换为旅行采购员问题的算例格式.
...
- **Checker/**
  旅行采购员问题计算结果检查程序的 Visual C++ 项目.

- **Analyzer/**
  旅行采购员问题算例/结果/日志分析程序的 Visual C# 项目.
...
1. 在 `Solver.cpp` 中搜索 `TODO[0]`, 并在对应位置添加求解算法的代码, 已初始化的相关数据结构用法参考 `optimize()` 函数和 `init()` 函数.
